...
If fso.FileExists(pywPath) And fso.FileExists(appPath) Then
    ...
    If exitCode <> 0 Then
        MsgBox "Snipit exited with code " & exitCode & "." & vbCrLf & _
               "Try running app.py manually to see the error.", vbExclamation, "Snipit Launcher"
    End If
Else
    MsgBox "Could not find .venv\Scripts\pythonw.exe or app.py." & vbCrLf & _
           "Set up the .venv and dependencies first.", vbExclamation, "Snipit Launcher"
End If